...
module.exports = (robot) ->
    ...
    pagerDutyIncidents msg, 'triggered,acknwowledged', (incidents) ->
      ...
      if filteredIncidents.length is 0
        ...
        if incidents.length > 0 and not force
          msg.send "Nothing assigned to you to acknowledge. Acknowledge someone else's incident with `/pager ack <nnn>`"
        else
  ...
  robot.respond /(pager|major)( me)? res(olve)?(d)?(!)?$/i, (msg) ->
    ...
    pagerDutyIncidents msg, "acknowledged", (incidents) ->
      ...
      if filteredIncidents.length is 0
        # nothing assigned to the user, but there were others
        if incidents.length > 0 and not force
          msg.send "Nothing assigned to you to resolve. Resolve someone else's incident with `/pager ack <nnn>`"
        else
  ...
  robot.respond /(pager|major)( me)? (schedule|overrides)/i, (msg) ->
    ...
    pagerDutyGet msg, "/schedules", {}, (json) ->
      for schedule in json.schedules
        pagerDutyGet msg, "/schedules/#{schedule.id}/#{thing}", query, (json) ->
  ...
  getUsersForIncident = (incident) ->
    if incident.assigned_to
      incident.assigned_to.map((user) -> user.object.email)
    else if incident.resolved_by
      incident.resolved_by.map((user) -> user.object.email)
    else
      ['(???)']
  ...
  generateIncidentString = (incident, hookType) ->
    ...
    assigned_user   = getUsersForIncident(incident).join(',')
    ...
    if hookType == "incident.trigger"
      ...
      #{incident.status} and assigned to #{assigned_user}
       #{incident.html_url}
      To acknowledge: /pager me ack #{incident_number}
      To resolve: /pager me resolve #{}
      """
      ...
      #{incident.status} and assigned to #{assigned_user}
       #{incident.html_url}
      To resolve: /pager me resolve #{incident_number}
      """
